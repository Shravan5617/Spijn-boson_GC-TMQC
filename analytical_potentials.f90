...
MODULE analytical_potentials
  ...
  SUBROUTINE new_model_potentials(Hel,grad_BO,NAC,Q)
    ...
    IF(model_potential=="NaI")                       &
       CALL NaI_potential(Hel,Q,grad_Hel)  
  ...
  SUBROUTINE NaI_potential(H,R,grad_H) 
 
    implicit none
  
    REAL*8, INTENT(INOUT)           :: H(NSTATES,NSTATES),      &
                                              grad_H(NSTATES,NSTATES,N_DOF)
    REAL*8, INTENT(IN)              :: R(N_DOF)
    ...
    real*8, parameter :: omg_c  = 1000.d0*omg2au
    real*8, parameter :: eta    = 400.d0*omg2au
    real*8, parameter :: gamma_B= eta
    real*8 :: lmda1  = 12000.d0*eng2au
    real*8 :: tempK  = 400.d0*tk2au
    real*8 ho_1, ho_2, omg(100) , c(100), H_b , grad_H_b
    integer :: i , k 
    real*8  :: f = 98.d0 
    real*8 :: L, S

        omg(1)   = 1200.d0*omg2au
        omg(2)   = 400.d0*omg2au


     c(1)   = dsqrt(0.5*lmda1*mp*omg(1)**2)
     c(2)   = 6.d-3 ! a.u.


 do i=3,100

   omg(i)=-omg_c*log((real(i)-5/2.d0)/(f))

   c(i)=omg(i)*(dsqrt(2*eta*mp*omg_c)/(f)*dacos(-1.d0))
 ...
 do i =3,100
   ...
   H_b=H_b +  (0.5* mp * omg(i)**2) * (R(i) - c(i)*R(2)/(mp*omg(i)**2))**2

 end do

        H = 0.d0
        ho_1 = (0.5* mp * omg(1)**2 * R(1)**2)
        ho_2 = (0.5* mp * omg(2)**2) * (R(2) - c(1)*R(1)/(mp*omg(2)**2))**2

        H(1,1) = en1 + ho_1 + c(1)*R(1)   +  ho_2 + H_b
        H(2,2) = en2 + ho_1 - c(1)*R(1)    + ho_2 + H_b
        H(1,2) = vcoup
        H(2,1) = H(1,2)


         grad_H = 0.d0
         grad_H(1,1,1) = mp * omg(1)**2 * R(1) + c(1)  - c(2)*R(2) + (c(2)**2)*R(1)/(mp*omg(2)**2)
         grad_H(2,2,1) = mp * omg(1)**2 * R(1) - c(1)  - c(2)*R(2) + (c(2)**2)*R(1)/(mp*omg(2)**2)



 do i =3,100

     grad_H_b=grad_H_b + (c(i)* R(i) - (c(i)**2)*R(2)/mp*omg(i)**2)

 end do

        grad_H(1,1,2)= mp * omg(2)**2 *R(2) - c(2)*R(1) +  grad_H_b
                                                                                   
        grad_H(2,2,2) = grad_H(1,1,2)

 do k = 1,2

 do  i =3,100

  grad_H(k,k,i)= mp*omg(i)**2*R(i) - c(i)*R(2)
 ...
 END SUBROUTINE NaI_potential
  ...
  !> @param[inout] H electronic Hamiltonian in the diabatic basis
  !> @param[inout] grad_H gradient of the electronic Hamiltonian in the
  !! diabatic basis  !> @param[in] R nuclear position
  ...
  SUBROUTINE plot_potential()
    ...
    REAL(KIND=dp)                  :: delta,R(n_dof),ground_state_density
    INTEGER                        :: ios,ix,i
    
    real*8, parameter ::  au2cm = 219474.6305
      ...
      DO ix=1,1000
        R=Rmin+delta*(ix-1)
              ...
              !  ground_state_density=SQRT(mass(1)*0.00134881_dp/PI)*               &
                ...
                WRITE(234,'(100f14.8)') R(1) ,V(1,1),V(1,2),V(2,2),   &
                                        epsBO,nacv(1,2,:),        &
                                        nacv(2,1,:),nacv(1,1,:),  &
                                        nacv(2,2,:)                       !,ground_state_density
                save_U=U

      write(235,*)  R(1), epsBO(1)*au2cm , epsBO(2)*au2cm 
