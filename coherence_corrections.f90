...
MODULE coherence_corrections
  ...
  SUBROUTINE quantum_momentum(Rcl,acc_force,BOsigma,k_li)
    ...
    ALLOCATE(gamma(n_dof,ntraj),g_i(ntraj),prod_g_i(ntraj,ntraj),     &
             ...
             denom(n_dof,npairs),qmom(n_dof,ntraj,npairs),            &
             threshold(n_dof))
    ...
    DO itraj=1,ntraj
      DO i_dof=1,n_dof
        ...
        DO istate=1,nstates
          DO jstate=istate+1,nstates
            index_ij=index_ij+1
            IF(dabs(num_old(i_dof,itraj,index_ij)-Rcl(itraj,i_dof))>M_parameter(i_dof)*threshold(i_dof)) THEN
              IF(dabs(num_new(i_dof,itraj,index_ij)-Rcl(itraj,i_dof))>M_parameter(i_dof)*threshold(i_dof)) THEN
